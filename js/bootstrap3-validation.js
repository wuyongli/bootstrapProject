...
!function($) {
   ...
   $.fn.validation.defaults = {
        validRules : [
            ...
            {name: 'name', validate: function(value) {return (!/^[A-Za-z0-9_]{5,15}$/.test(value));}, defaultMsg: '请输入5-15位字符，支持英文、数字、"_"组合。'},
            ...
            {name: 'tel', validate:function(value){return(!/^1[3|4|5|7|8][0-9]\d{4,8}$/.test(value));}, defaultMsg: '请输入电话号码'}
        ],
        reqmark:true,
    ...
    var validatePasswordField = function() {
    	...
    	if ($.trim(el.val()) == '') {
    		...
    		if($.trim(password.val()) != $.trim(el.val())) {
        		error = true;
        		errorMsg = '两次密码输入不一致！';
        	}
    	}
